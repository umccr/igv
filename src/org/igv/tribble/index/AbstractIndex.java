...
import org.igv.tribble.Tribble;
 ...
 * @author jrobinso
 */
public abstract class AbstractIndex implements org.igv.tribble.index.MutableIndex {
     ...
     */
    protected LinkedHashMap<String, org.igv.tribble.index.ChrIndex> chrIndices;
    ...
    public AbstractIndex() {
        ...
        chrIndices = new LinkedHashMap<String, org.igv.tribble.index.ChrIndex>();
    }
    ...
    public List<org.igv.tribble.index.Block> getBlocks(final String chr, final int start, final int end) {
        return getChrIndex(chr).getBlocks(start, end);
    }

    public List<org.igv.tribble.index.Block> getBlocks(final String chr) {
     ...
     */
    private final org.igv.tribble.index.ChrIndex getChrIndex(final String chr) {
        final org.igv.tribble.index.ChrIndex chrIdx = chrIndices.get(chr);
        if (chrIdx == null) {
    ...
    public void write(final LittleEndianOutputStream stream) throws IOException {
        ...
        for (final org.igv.tribble.index.ChrIndex chrIdx : chrIndices.values()) {
            chrIdx.write(stream);
        }
    }
    ...
    public void read(final LittleEndianInputStream dis) throws IOException {
        try {
            ...
            chrIndices = new LinkedHashMap<String, org.igv.tribble.index.ChrIndex>(nChromosomes);

            while (nChromosomes-- > 0) {
                final org.igv.tribble.index.ChrIndex chrIdx = (org.igv.tribble.index.ChrIndex) getChrIndexClass().newInstance();
                chrIdx.read(dis);
    ...
    protected BlockStats getBlockStats(final boolean logDetails) {
        ...
        for (final Map.Entry<String, ChrIndex> elt : chrIndices.entrySet()) {
            final List<org.igv.tribble.index.Block> blocks = elt.getValue().getBlocks();
