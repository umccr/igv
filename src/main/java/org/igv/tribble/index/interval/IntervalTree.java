...
public class IntervalTree {
    ...
    public void insert(org.igv.tribble.index.interval.Interval interval) {
     ...
     */
    public List<org.igv.tribble.index.interval.Interval> findOverlapping(org.igv.tribble.index.interval.Interval interval) {

        if (root().isNull()) {
            return Collections.emptyList();
        }

        List<org.igv.tribble.index.interval.Interval> results = new ArrayList<org.igv.tribble.index.interval.Interval>();
    ...
    private List<org.igv.tribble.index.interval.Interval> searchAll(org.igv.tribble.index.interval.Interval interval, Node node, List<org.igv.tribble.index.interval.Interval> results) {
     ...
     * @return
     */
    public List<org.igv.tribble.index.interval.Interval> getIntervals() {
        if (root().isNull()) {
            return Collections.emptyList();
        }
        List<org.igv.tribble.index.interval.Interval> results = new ArrayList<org.igv.tribble.index.interval.Interval>(size);
        getAll(root(), results);
        return results;
    }
     ...
     */
    private List<org.igv.tribble.index.interval.Interval> getAll(Node node, List<org.igv.tribble.index.interval.Interval> results) {
    ...
    private int getRealMax(Node node) {
        ...
        int nodeHigh = (int) (node.interval).end;
    ...
    private int getRealMin(Node node) {
        ...
        int nodeLow = (int) (node.interval).start;
    ...
    private void update(Node node) {
        node.max = (int) Math.max(Math.max(node.left.max, node.right.max), node.interval.end);
        node.min = (int) Math.min(Math.min(node.left.min, node.right.min), node.interval.start);
    }
    ...
    static class Node {
        ...
        org.igv.tribble.index.interval.Interval interval;
        int min;
        int max;
        Node left;
        Node right;
        ...
        public void store(DataOutputStream dos) throws IOException {
            dos.writeInt((int) interval.start);
            dos.writeInt((int) interval.end);
            dos.writeInt(min);
            dos.writeInt(max);

        }

        public Node(org.igv.tribble.index.interval.Interval interval) {
            this();
            this.parent = NIL;
            this.left = NIL;
            this.right = NIL;
        ...
        public String toString() {
            ...
            Map<org.igv.tribble.index.interval.Interval, Integer> keys = new LinkedHashMap<org.igv.tribble.index.interval.Interval, Integer>();
            ...
            buf.append('\n');
            for (Map.Entry<org.igv.tribble.index.interval.Interval, Integer> entry : keys.entrySet()) {
        ...
        public void _toString(StringBuffer buf, Map<org.igv.tribble.index.interval.Interval, Integer> keys) {
            if (this == NIL) {
                buf.append("nil");
                buf.append('\n');
                return;
